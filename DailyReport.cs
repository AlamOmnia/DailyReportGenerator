...
{
    /// <summary>
    /// This program assumes that all the data are between A-Z column of Excel
    /// </summary>
    class DailyReport
    {
        private const char DELIMITER = '\t';
        private const string REPORT_TEMPLATE_FILE = @"D:/C#/MicroSoft Visual Studio 2013/Demo_Excel_Export/bin/Release/Reports/BTRC_Daily_Report_Template.xlsx";
        //private const string REPORT_SHEET_NAME = "Sheet1";
        private const string ADDRESS_OF_IGW_FIRST_CELL = "B12";
        private const string ADDRESS_OF_TOTAL = "A41";
        private const string ADDRESS_OF_DATE = "F8";
        private const int IGW_COUNT = 29;
        ...
        public DailyReport(DataTable incomingDataTable, DataTable outgoingDataTable, string  reportDate)
        {
        ...
        public void ExportToExcel(string outputFile)
        {
            ...
            using (SpreadsheetDocument spreadSheetDocument = SpreadsheetDocument.Open(outputFile, true))
            {
                ...
                #region Set totals
                string incomingTotalCallCountFirstCell = GetNextColumnCellReference(ADDRESS_OF_IGW_FIRST_CELL, 1);  // C12
                string incomingTotaLMinsFirstCell = GetNextColumnCellReference(ADDRESS_OF_IGW_FIRST_CELL, 2);  // D12
                string outgoingTotalCallCountFirstCell = GetNextColumnCellReference(ADDRESS_OF_IGW_FIRST_CELL, 3);  // E12
                string outgoingTotaLMinsFirstCell = GetNextColumnCellReference(ADDRESS_OF_IGW_FIRST_CELL, 4);  // F12

                // Totals
                string incomingTotalCallCountSum = GetNextRowCellReference(incomingTotalCallCountFirstCell, IGW_COUNT);  // C41
                string incomingTotalMinsSum = GetNextRowCellReference(incomingTotaLMinsFirstCell, IGW_COUNT);  // D41
                string outgoingTotalCallCountSum = GetNextRowCellReference(outgoingTotalCallCountFirstCell, IGW_COUNT);  // E41
                string outgoingTotalMinsSum = GetNextRowCellReference(outgoingTotaLMinsFirstCell, IGW_COUNT);  // F41


                ExcelHelper.Instance.CalculateSumOfCellRange(workbookPart,
                    worksheetPart,
                    incomingTotalCallCountFirstCell,           // C12
                    GetNextRowCellReference(incomingTotalCallCountFirstCell, IGW_COUNT - 1),    // C40
                    incomingTotalCallCountSum);      // C41 (SUM of c12:c40)
        ...
        private bool IsInIgwColumn(string cellReference)
        {
            ...
            // check the cell is between B12 and B40
